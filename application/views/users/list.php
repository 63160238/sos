                ...
                <?php foreach ($getData as $key => $value) : ?>
                  <tr>
                    ...
                    <?php foreach ($apartment_name as $key => $value2) : ?>
                      <?php if ($value->user_a_id == $value2->a_id) : ?>
                        <td><?= $value2->a_name ?></td>
                      <?php endif; ?>
                    <?php endforeach; ?>
                    <?php else : ?>
                    ...
                    <td class="text-center">
                      <button type="button" class="btn btn-outline-secondary btn-sm" name="edit" id="edit" onclick="edit(<?= $value->user_id ?>)" title="แก้ไขข้อมูลพนักงาน"><i class="icon-pencil"></i></button>
                      ...
                      <?php if ($value->status == 1) : ?>
                        <button type="button" class="btn btn-outline-secondary btn-sm" name="del" id="del" title="ปิดการใช้งาน" onclick="changeStatus(<?= $value->user_id ?>,<?= $value->status ?>)"><i class="fas fa-lock-open"></i></button>
                      <?php else : ?>
                        <button type="button" class="btn btn-outline-secondary btn-sm" name="del" id="del" title="เปิดใช้งาน" onclick="changeStatus(<?= $value->user_id ?>,<?= $value->status ?>)"><i class="fas fa-lock"></i></button>
                      <?php endif; ?>
                    </td>
                  </tr>
                <?php endforeach; ?>
