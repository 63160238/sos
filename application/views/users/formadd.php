        ...
        <div class="card-body">
          <h6 class="card-subtitle">กรุณากรอกข้อมูลให้ครบถ้วน</h5>
            ...
            <div class="form-group" id="c_a">
              <label for="user_a_id">หอพัก<span class="text-danger"> *</span></label>
              <select class="form-control" id="user_a_id" name="inputValue[]">
                <?php foreach ($apartment_name as $key => $value) : ?>
                  <option value="<?= $value->a_id ?>" <?= isset($getData) && is_object($getData) && property_exists($getData, 'a_id') && $getData->a_id == $value->a_id ? 'selected' : '' ?>>
                    <?= $value->a_name ?>
                  </option>
                <?php endforeach; ?>
              </select>
